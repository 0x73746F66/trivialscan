...
VALIDATION_ERROR_VERSION_INTERFERENCE_CURRENT = 'Your server is not configured to accept clients that only support the current TLS version {current_version} and these clients browsers configured for privacy (and/or highest security) will be shown am error "ERR_SSL_VERSION_INTERFERENCE" and not negotiate a secure connection to your server.'
VALIDATION_ERROR_VERSION_INTERFERENCE_COMMON = 'Your server is not configured to accept clients that only support the common common TLS version {common_version} (You may be limited to older or only the newest versions) and these client browsers not yet configured for the latest TLS version {current_version} will be shown am error "ERR_SSL_VERSION_INTERFERENCE" and not negotiate a secure connection to your server.'
VALIDATION_ERROR_VERSION_INTERFERENCE_OLD = 'Your server is not configured to accept clients that only support TLS version {old_version} or older, these clients will be shown am error "ERR_SSL_VERSION_INTERFERENCE" and not negotiate a secure connection to your server.'
VALIDATION_ERROR_VERSION_INTERFERENCE_OBSOLETE = 'Your server is not configured to accept clients that only support obsolete TLS versions, these clients will be shown am error "ERR_SSL_VERSION_INTERFERENCE" and not negotiate a secure connection to your server.'
