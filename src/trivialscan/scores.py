...
from trivialscan import Validator, LeafCertValidator, RootCertValidator, PeerCertValidator
...
class Score:
    ...
    def risk_summary(self) -> list[str]:
        ...
        for validator in self._validators:
            if validator.metadata.certificate_known_compromised:
                issues.add(f'Certificate is known to be compromised; {validator.certificate.subject.rfc4514_string()} ({validator.metadata.certificate_sha1_fingerprint})')
            if validator.metadata.possible_phish_or_malicious:
                issues.add('Connection with server is potentially malicious (e.g. phish)')
            if isinstance(validator, LeafCertValidator) and validator.metadata.revocation_ocsp_must_staple:
                mitigations.add('Certificate uses the OCSP must staple flag which provides a high assurance of revocation controls')
            if isinstance(validator, LeafCertValidator) and validator.metadata.revocation_ocsp_stapling:
                mitigations.add('Certificate uses the OCSP stapling which provides limited revocation controls')
            if isinstance(validator, LeafCertValidator) and validator.metadata.http_expect_ct_report_uri:
                mitigations.add('Connection included Certificate Transparency reporting configuration')
        if not issues:
    ...
    def trust_summary(self) -> str:
        ...
        for v in self._validators:
            if not isinstance(v, RootCertValidator):
                continue
            any_trust = any([v.metadata.trust_ccadb, v.metadata.trust_android,
                            v.metadata.trust_linux, v.metadata.trust_certifi, v.metadata.trust_java])
            if not any_trust:
        ...
        for v in self._validators:
            ...
            validation_type = constants.VALIDATION_OID[v.metadata.certificate_validation_oid]
            if validation_type == 'DV' and v.metadata.certificate_issuer in constants.QUESTIONABLE_DV_ISSUERS:
        ...
        for v in self._validators:
            if not isinstance(v, RootCertValidator):
                continue
            is_trusted = all([v.metadata.trust_ccadb, v.metadata.trust_android,
                            v.metadata.trust_linux, v.metadata.trust_certifi, v.metadata.trust_java])
        for v in self._validators:
            if not isinstance(v, LeafCertValidator):
                continue
            if VALIDATION_VALID_CAA in v.validation_checks and is_trusted and v.validation_checks[VALIDATION_VALID_CAA]:
                return constants.VERIFIED_TRUST
            if is_trusted and (VALIDATION_VALID_CAA not in v.validation_checks or v.validation_checks[VALIDATION_VALID_CAA] is False):
    ...
    def key_size(metadata :Metadata) -> int:
        if metadata.certificate_key_compromised:
            return ISSUE_MOD
        if metadata.certificate_public_key_size > constants.WEAK_KEY_SIZE[metadata.certificate_public_key_type]*3:
            return MAJOR_MOD
        if metadata.certificate_public_key_size > constants.WEAK_KEY_SIZE[metadata.certificate_public_key_type]*2:
            return MINOR_MOD
        return WEAKNESS_MOD if metadata.certificate_public_key_size == constants.WEAK_KEY_SIZE[metadata.certificate_public_key_type] else ISSUE_MOD

    @staticmethod
    def server_configuration(metadata: Metadata) -> int:
        if metadata.tls_version_intolerance and constants.TLS1_3_LABEL in metadata.tls_version_intolerance_versions:
            return ISSUE_MOD
    ...
    def trust(metadata: Metadata) -> int:
        if all([metadata.trust_ccadb, metadata.trust_java, metadata.trust_android, metadata.trust_linux, metadata.trust_certifi]):
            return MAJOR_MOD
        if metadata.trust_ccadb:
            return MINOR_MOD
        return WEAKNESS_MOD if any([metadata.trust_ccadb, metadata.trust_java, metadata.trust_android, metadata.trust_linux, metadata.trust_certifi]) else ISSUE_MOD
    ...
    def dnssec(metadata :Metadata) -> int:
        if metadata.dnssec and metadata.dnssec_algorithm not in constants.WEAK_DNSSEC_ALGORITHMS.keys():
            return MAJOR_MOD
        if not metadata.dnssec or metadata.dnssec_algorithm in constants.WEAK_DNSSEC_ALGORITHMS.keys():
            return ISSUE_MOD
        if metadata.tlsa:
            return MINOR_MOD
        return WEAKNESS_MOD
    ...
    def cipher(metadata :Metadata) -> int:
        ...
        if metadata.weak_cipher:
            return ISSUE_MOD
        if metadata.forward_anonymity or metadata.negotiated_cipher_bits >= WEAK_CIPHER_BITS:
            return MINOR_MOD
        return WEAKNESS_MOD
    ...
    def certificate_validation(metadata :Metadata) -> int:
        ...
        if validation_type == 'EV':
            return MINOR_MOD
        if validation_type == 'DV' and metadata.certificate_issuer in constants.QUESTIONABLE_DV_ISSUERS:
            return ISSUE_MOD
