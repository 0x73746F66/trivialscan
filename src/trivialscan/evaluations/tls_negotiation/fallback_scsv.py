...
class EvaluationTask(BaseEvaluationTask):
    ...
    def evaluate(self) -> bool | None:
        ...
        if self._transport.state.preferred_protocol not in [
            TLS1_3_LABEL,
            self._transport.state.negotiated_protocol,
        ]:
            self._supports_fallback_scsv = False
        # should only occur if using only SSL3
        if self._transport.state.preferred_protocol not in PROTOCOL_TEXT_MAP.keys():
            return None
        ...
        ctx.set_max_proto_version(
            PROTOCOL_TEXT_MAP[self._transport.state.preferred_protocol]
        )
        ...
        if ssl.HAS_SNI:
            conn.set_tlsext_host_name(idna.encode(self._transport.state.hostname))
        try:
            conn.connect((self._transport.state.hostname, self._transport.state.port))
