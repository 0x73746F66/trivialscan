...
def _make_table(result :validator.Validator, title :str, caption :str) -> Table:
    title_style = Style(bold=True, color=config.CLI_COLOR_OK if result.certificate_valid else config.CLI_COLOR_NOK)
...
def _table_data(result :validator.Validator, table :Table, skip :list[str]) -> Table:
    ...
    for key in result.validation_checks.keys():
        if key in skip:
            continue
        table.add_row(config.STYLES.get(key,{}).get('text', key), util.styled_boolean(result.validation_checks[key], config.STYLES[key]['represent_as'], config.STYLES[key]['colors']))
    for key in result.compliance_checks.keys():
        if key in skip:
            continue
        table.add_row(config.STYLES.get(key,{}).get('text', key), util.styled_boolean(result.compliance_checks[key], config.STYLES[key]['represent_as'], config.STYLES[key]['colors']))
    for key in list(vars(result.metadata).keys()):
        ...
        if key in config.FINGERPRINTS and isinstance(val, str):
            table.add_row(config.STYLES.get(key,{}).get('text', key), util.str_n_split(val).upper())
            continue
        if val is None or (isinstance(val, str) and len(val) == 0):
            table.add_row(config.STYLES.get(key,{}).get('text', key), util.styled_value(config.STYLES[key].get('null_as', 'Unknown'), color=config.STYLES[key].get('null_color', config.CLI_COLOR_NULL)))
            continue
        if isinstance(val, str) and len(val) > 0:
            table.add_row(config.STYLES.get(key,{}).get('text', key), util.styled_value(val, color=config.STYLES[key].get('color')))
            continue
        if isinstance(val, bool):
            table.add_row(config.STYLES.get(key,{}).get('text', key), util.styled_boolean(val, represent_as=config.STYLES[key]['represent_as'], colors=config.STYLES[key]['colors']))
            continue
...
def table_score(score_card :Score, rating_color :str, target :str) -> Table:
    ...
    table.add_column(
        f"Security Score Card {target}", justify="right", style="dark_turquoise", no_wrap=True)
    table.add_column("", justify="left", no_wrap=False)
    table.add_row('Rating', util.styled_any(config.RATING_ASCII[score_card.rating]), style=rating_color)
...
def peer_outputs(result :validator.PeerCertValidator) -> Table:
    ...
    table.add_row(config.STYLES['certificate_valid']['text'], util.styled_boolean(result.certificate_valid, config.STYLES['certificate_valid']['represent_as'], config.STYLES['certificate_valid']['colors']))
...
def root_outputs(result :validator.RootCertValidator) -> Table:
    ...
    table = _make_table(result, title, caption)
    table.add_row(config.STYLES['certificate_valid']['text'], util.styled_boolean(result.certificate_valid, ('Trusted', 'Not Trusted'), config.STYLES['certificate_valid']['colors']))
...
def server_outputs(result :validator.LeafCertValidator) -> Table:
    ...
    table = _make_table(result, title, caption)
    table.add_row(config.STYLES['certificate_valid']['text'], util.styled_boolean(result.certificate_valid, config.STYLES['certificate_valid']['represent_as'], config.STYLES['certificate_valid']['colors']))
    table.add_row(config.STYLES['certificate_chain_valid']['text'], util.styled_boolean(result.certificate_chain_valid, config.STYLES['certificate_chain_valid']['represent_as'], config.STYLES['certificate_chain_valid']['colors']))
    table.add_row(config.STYLES['certificate_chain_validation_result']['text'], util.styled_any(result.certificate_chain_validation_result))
...
def prepare_json(results :list[validator.Validator], target :str) -> dict:
    ...
    for result in results:
        if isinstance(result, validator.RootCertValidator):
            data['validations'].append(validator_data(result, 'Root CA', [x for x in config.ROOT_SKIP if x not in config.JSON_ONLY]))
        ...
        if isinstance(result, validator.PeerCertValidator):
            ...
            data['validations'].append(validator_data(result, cert_type, [x for x in config.PEER_SKIP if x not in config.JSON_ONLY]))
        if isinstance(result, validator.LeafCertValidator):
            data['validations'].append(validator_data(result, 'Leaf Certificate', [x for x in config.SERVER_SKIP if x not in config.SERVER_JSON_ONLY]))
    return data
