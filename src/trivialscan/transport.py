...
class Transport:
    ...
    def pre_client_authentication_check(self, client_pem_path: str = None) -> bool:
        if not isinstance(self.port, int):
            raise TypeError(
                f"provided an invalid type {type(self.port)} for port, expected int"
            )
        ...
        if valid_client_pem is False:
            logger.error(
                "client_pem_path was provided but is not a valid URL or file does not exist"
            )
            return False
        if isinstance(valid_client_pem, list) and len(valid_client_pem) == 1:
            self.client_pem_path = valid_client_pem[0]
        logger.info(
            "Negotiating with the server to derive expected client certificate subjects"
        )
        ...
        if len(self.expected_client_subjects) > 0:
            ...
            self.client_certificate = load_certificate(
                FILETYPE_PEM, Path(self.client_pem_path).read_bytes()
            )
            logger.debug(
                f"issuer subject: {self.client_certificate.get_issuer().commonName}"
            )
    ...
    def _ocsp_handler(self, conn: SSL.Connection, assertion: bytes, userdata) -> bool:
        ...
        if response.response_status.value in constants.OCSP_RESP_STATUS:
            self.ocsp_response_status = constants.OCSP_RESP_STATUS[
                response.response_status.value
            ]
        if response.certificate_status.value in constants.OCSP_CERT_STATUS:
            self.ocsp_certificate_status = constants.OCSP_CERT_STATUS[
    ...
    def prepare_context(
        ...
        if self.client_certificate_expected and isinstance(self.client_pem_path, str):
            ctx.use_certificate_file(
                certfile=self.client_pem_path, filetype=FILETYPE_PEM
            )
    ...
    def prepare_connection(
        ...
        conn = SSL.Connection(
            context,
            ctx.wrap_socket(
                sock, do_handshake_on_connect=False, server_hostname=self.host
            ),
        )
        conn.connect((self.host, self.port))
        conn.set_connect_state()
    ...
    def decode_http2_frame(frame_data) -> Frame:
        try:
            ...
            f.parse_body(memoryview(frame_data[9 : 9 + length]))  # noqa: E203
        except InvalidFrameError:
            f = None
        return f

    def _http2(
        self, conn: SSL.Connection, request: Frame, response_wait: int = 3
    ) -> Frame:
        ...
        while True:
            try:
                if (
                    Transport.is_connection_closed(conn)
                    or (datetime.utcnow() - start).seconds >= response_wait
                ):
                    break
                data = conn.recv(1024)
                if not data:
    ...
    def test_h2c(
        ...
        logger.info("Testing HTTP/2 clear text")
        ...
        if include_settings:
            ...
            http2_settings = urlsafe_b64encode(settings_frame.serialize_body()).rstrip(
                b"="
            )
        ...
        try:
            ...
            if include_settings:
                logger.info(
                    f"HTTP2-Settings: {Transport.decode_http2_frame(settings_frame.serialize())}"
                )
    ...
    def test_http2(self, uri_path: str = "/", response_wait: int = 3):
        ...
        conn = self.prepare_connection(
            context=ctx, sock=self.prepare_socket(response_wait)
        )
        conn.settimeout(response_wait)
        util.do_handshake(conn)
        request = Transport.decode_http2_frame(
            b"\x00\x00\x13\x00\x09\x00\x00\x00\x01\x0Ahello" + b"\0" * 10
        )
    ...
    def test_highest_tls_version(self, version, response_wait: int = 3):
        logger.warning(
            DeprecationWarning(
                "Transport.test_highest_tls_version() was deprecated in version 0.4.4 and will be removed in version 0.5.0"
            ),
            exc_info=True,
        )
    ...
    def connect(self, tls_version: int, use_sni: bool = False, protocol: str = None):
        logger.info(
            f"Trying TLS version {constants.OPENSSL_VERSION_LOOKUP[tls_version]}"
        )
        ctx = self.prepare_context()
        ctx.set_verify(
            getattr(SSL, Transport.default_connect_verify_mode), self._verifier
        )
    ...
    def test_tls_all_versions(self, use_sni: bool = True):
        ...
        for ver_name, tls_version in constants.PROTOCOL_VERSION.items():
            ...
            if (
                ver_name in FAKE_PROTOCOLS
                or ver_display_name in self.offered_tls_versions
            ):
                continue
            negotiated = self.test_tls_version(
                max_tls_version=tls_version, use_sni=use_sni
            )
    ...
    def test_tls_version_intolerance(self, use_sni: bool = True):
        ...
        for fake_proto in FAKE_PROTOCOLS:
            ...
            try:
                intolerance = (
                    self.test_tls_version(min_tls_version=fake_ver, use_sni=use_sni)
                    is None
                )
                if intolerance:
                    self.tls_version_intolerance_versions.append(
                        f"{fake_proto} ({hex(fake_ver)})"
                    )
            except Exception:
                self.tls_version_intolerance_versions.append(
                    f"{fake_proto} ({hex(fake_ver)})"
    ...
    def connect_least_secure(self, cafiles: list = None, use_sni: bool = False) -> bool:
        if not isinstance(self.port, int):
            raise TypeError(
                f"provided an invalid type {type(self.port)} for port, expected int"
            )
        ...
        for index, version in enumerate(tls_versions):
            ...
            self.long_handshake_intolerance = self.test_tls_long_handshake_intolerance(
                version
            )
            if version == SSL.TLS1_3_VERSION:
                ...
                self.test_tls_version_intolerance(
                    use_sni
